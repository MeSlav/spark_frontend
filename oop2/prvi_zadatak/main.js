...
Video.prototype.watch = function(amount) {
    if(amount) {
        console.log(`You watched ${amount} of ${this.uploader}'s ${this.title}.`);
    } else {
        console.log(`You watched all ${this.duration} of ${this.uploader}'s ${this.title}.`);
    }
}
...
function MusicVideo(title, uploader, duration, artist) {
    Video.call(this, title, uploader, duration);
    this.artist = artist;
}

MusicVideo.prototype.rockOut = function() {
    console.log(`You rocked out ${this.title} of ${this.uploader}.`);
}

const kanyeWestVids = [
    {
        title: 'Fade',
        uploader: 'KanyeWestVEVO',
        duration: '3 minutes',
        artist: 'Kanye West',
    },
    {
...
multipleVideos.forEach(({
    ...
    if (artist) {
        const musicVid = new MusicVideo(title, uploader, duration, artist);
        musicVid.rockOut();
    } else {
        const movie = new Video(title, uploader, duration);
        movie.watch();
    }
})